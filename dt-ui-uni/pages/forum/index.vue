<template>
	<view v-if="pageLoad">
		...
		<view class="tabs-border">
			...
			<view @click="setCat(item.gid)" v-bind:class="{ 'tabs-border-active':item.isactive }" v-for="(item,key) in pageData.grouplist" :key="key" class="tabs-border-item">{{item.title}}</view>
	...
	export default{
		...
		methods:{
			getPage:function(){
				var that=this;
				uni.request({
					url:app.apiHost+"?m=forum&ajax=1",
					data:{
			...
			getList:function(){
				...
				uni.request({
					url:app.apiHost+"?m=forum&ajax=1",data:{
						per_page:per_page,
						catid:catid,
						authcode:app.getAuthCode()
					},
